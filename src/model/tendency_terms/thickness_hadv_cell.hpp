...
struct ThicknessHorzAdvOnCell {
  ...
  KOKKOS_FUNCTION void operator()(const Real2d &h_tend_cell, Int icell,
    ...
    for (Int j = 0; j < m_nedges_on_cell(icell); ++j) {
      ...
      accum -= m_dv_edge(jedge) * inv_area_cell *
               m_edge_sign_on_cell(icell, j) * h_flux_jedge * vn_jedge;
    }
    ...
    for (Int j = 0; j < m_nedges_on_cell(icell); ++j) {
      ...
      for (Int kvec = 0; kvec < vector_length; ++kvec) {
        ...
        accum[kvec] -= m_dv_edge(jedge) * inv_area_cell *
                       m_edge_sign_on_cell(icell, j) * h_edge(jedge, k) *
                       v_edge(jedge, k);
      }
    }
    OMEGA_SIMD_PRAGMA

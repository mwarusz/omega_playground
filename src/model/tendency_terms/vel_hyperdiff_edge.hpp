...
struct VelocityHyperDiffusionOnEdge {
  ...
  void enable(ShallowWaterAuxiliaryState &aux_state) {
    m_enabled = true;
    aux_state.m_vel_del2_div_cell.enable(aux_state);
    aux_state.m_vel_del2_rvort_vertex.enable(aux_state);
  }

  KOKKOS_FUNCTION Real operator()(Int iedge, Int k,
                                  const RealConst2d &del2_div_cell,
                                  const RealConst2d &del2_rvort_vertex) const {
    ...
    const Real del2u =
        ((del2_div_cell(icell1, k) - del2_div_cell(icell0, k)) * dc_edge_inv -
         (del2_rvort_vertex(ivertex1, k) - del2_rvort_vertex(ivertex0, k)) *
             dv_edge_inv);
  ...
  VelocityHyperDiffusionOnEdge(const MPASMesh *mesh, Real visc_del4)
      : m_cells_on_edge(mesh->m_cells_on_edge),
        ...
        m_edge_mask(mesh->m_edge_mask), m_visc_del4(visc_del4) {}
};

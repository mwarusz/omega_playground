...
struct NormRelVortOnEdge {
  ...
  KOKKOS_FUNCTION Real operator()(Int iedge, Int k,
                                  const RealConst2d &norm_rvort_vertex) const {
    Int jvertex0 = m_vertices_on_edge(iedge, 0);
    Int jvertex1 = m_vertices_on_edge(iedge, 1);
    return 0.5_fp *
           (norm_rvort_vertex(jvertex0, k) + norm_rvort_vertex(jvertex1, k));
  }

  NormRelVortOnEdge(const MPASMesh *mesh)

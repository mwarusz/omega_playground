...
Real ShallowWaterBase::mass_integral(RealConst2d h) const {
  ...
  parallel_for("compute_column_mass", mesh->ncells, YAKL_LAMBDA (Int icell) {
      column_mass(icell) = 0;
...
Real ShallowWaterBase::circulation_integral(RealConst2d v) const {
  ...
  parallel_for("compute_column_circulation", mesh->nvertices, YAKL_LAMBDA (Int ivertex) {
      column_circulation(ivertex) = 0;
      for (Int k = 0; k < max_level_vertex_bot(ivertex); ++k) {
        Real cir_i = -0;
        for (Int j = 0; j < 3; ++j) {
          Int jedge = edges_on_vertex(ivertex, j);
          cir_i += dc_edge(jedge) * orient_on_vertex(ivertex, j) * v(jedge, k);
        }
...
ShallowWater::ShallowWater(PlanarHexagonalMesh &mesh, Real f0, Real grav) :
  ShallowWaterBase(mesh, f0, grav), hflux("hflux", mesh.nedges, mesh.nlayers) {}

void ShallowWater::compute_h_tendency(Real2d htend, RealConst2d h, RealConst2d v) const {
  ...
  parallel_for("compute_htend", mesh->ncells, YAKL_LAMBDA (Int icell) {
      for (Int k = 0; k < max_level_cell(k); ++k) {
        ...
        for (Int j = 0; j < nedges_on_cell(icell); ++j) {
          ...
          accum += dv_edge(jedge) * orient_on_cell(icell, j) * hflux(jedge, k);
        }
...
void ShallowWater::compute_v_tendency(Real2d vtend, RealConst2d h, RealConst2d v) const {
  YAKL_SCOPE(edges_on_vertex, mesh->edges_on_vertex);
  YAKL_SCOPE(orient_on_vertex, mesh->orient_on_vertex);
  ...
  parallel_for("compute_vtend", mesh->nedges, YAKL_LAMBDA (Int iedge) {
      for (Int k = 0; k < max_level_edge_top(iedge); ++k) {
        ...
        for (Int j = 0; j < nedges_on_edge(iedge); ++j) {
          ...
          qt += weights_on_edge(iedge, j) * dv_edge(jedge) * hflux(jedge, k) * (qe(iedge, k) + qe(jedge, k)) / 2;
        }
        qt /= dc_edge(iedge);
        ...
        Real grad_B = (K(icell1, k) - K(icell0, k) + grav * (h(icell1, k) - h(icell0, k))) / dc_edge(iedge);
...
Real ShallowWater::energy_integral(RealConst2d h, RealConst2d v) const {
  ...
  parallel_for("compute_column_energy", mesh->ncells, YAKL_LAMBDA (Int icell) {
      ...
      for (Int k = 0; k < max_level_cell(icell); ++k) {
        ...
        K /= area_cell(icell);
        column_energy(icell) += area_cell(icell) * (grav * h(icell, k) * h(icell, k) / 2 + h(icell, k) * K);
      }
  });
...
void LinearShallowWater::compute_h_tendency(Real2d htend, RealConst2d h, RealConst2d v) const {
  YAKL_SCOPE(nedges_on_cell, mesh->nedges_on_cell);
  YAKL_SCOPE(edges_on_cell, mesh->edges_on_cell);
  YAKL_SCOPE(dv_edge, mesh->dv_edge);
  YAKL_SCOPE(orient_on_cell, mesh->orient_on_cell);
...
void LinearShallowWater::compute_v_tendency(Real2d vtend, RealConst2d h, RealConst2d v) const {
  YAKL_SCOPE(nedges_on_edge, mesh->nedges_on_edge);
  YAKL_SCOPE(edges_on_edge, mesh->edges_on_edge);
  YAKL_SCOPE(weights_on_edge, mesh->weights_on_edge);
  YAKL_SCOPE(dv_edge, mesh->dv_edge);
...
Real LinearShallowWater::energy_integral(RealConst2d h, RealConst2d v) const {
  ...
  parallel_for("compute_column_energy", mesh->ncells, YAKL_LAMBDA (Int icell) {
      ...
      for (Int k = 0; k < max_level_cell(icell); ++k) {
        ...
        K /= area_cell(icell);
        column_energy(icell) += area_cell(icell) * (grav * h(icell, k) * h(icell, k) / 2 + h0 * K);
      }
  });
  return yakl::intrinsics::sum(column_energy);
}

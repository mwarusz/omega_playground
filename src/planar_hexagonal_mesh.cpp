...
YAKL_INLINE Int PlanarHexagonalMesh::cell_on_cell(Int icol, Int irow, Int nb) const {
  ...
  if (irow % 2 == 0) {
    if (nb == 0) { return cellidx(mx, irow); }
    if (nb == 1) { return cellidx(mx, my); }
    if (nb == 2) { return cellidx(icol, my); }
    if (nb == 3) { return cellidx(px, irow); }
    if (nb == 4) { return cellidx(icol, py); }
    if (nb == 5) { return cellidx(mx, py); }
  } else {
    if (nb == 0) { return cellidx(mx, irow); }
    if (nb == 1) { return cellidx(icol, my); }
    if (nb == 2) { return cellidx(px, my); }
    if (nb == 3) { return cellidx(px, irow); }
    if (nb == 4) { return cellidx(px, py); }
    if (nb == 5) { return cellidx(icol, py); }
  }
  return -1;
}

YAKL_INLINE Int PlanarHexagonalMesh::edge_on_cell(Int icell, Int icol, Int irow, Int nb) const {
  if (nb == 0) { return 3 * icell; }
  if (nb == 1) { return 3 * icell + 1; }
  if (nb == 2) { return 3 * icell + 2; }
  if (nb == 3) { return 3 * cell_on_cell(icol, irow, nb); }
  if (nb == 4) { return 3 * cell_on_cell(icol, irow, nb) + 1; }
  if (nb == 5) { return 3 * cell_on_cell(icol, irow, nb) + 2; }
  return -1;
}

YAKL_INLINE Int PlanarHexagonalMesh::vertex_on_cell(Int icell, Int icol, Int irow, Int nb) const {
  if (nb == 0) { return 2 * icell; }
  if (nb == 1) { return 2 * icell + 1; }
  if (nb == 2) { return 2 * cell_on_cell(icol, irow, 2); }
  if (nb == 3) { return 2 * cell_on_cell(icol, irow, 3) + 1; }
  if (nb == 4) { return 2 * cell_on_cell(icol, irow, 3); }
  if (nb == 5) { return 2 * cell_on_cell(icol, irow, 4) + 1; }
  return -1;
}

void PlanarHexagonalMesh::compute_mesh_arrays() {
  parallel_for("compute_mesh_arrays", 
      SimpleBounds<2>(ny, nx), YAKL_CLASS_LAMBDA(Int irow, Int icol) {
      ...
      vertices_on_edge(edges_on_cell(icell, 0), 0) = vertices_on_cell(icell, 1);
      vertices_on_edge(edges_on_cell(icell, 0), 1) = vertices_on_cell(icell, 0);
      vertices_on_edge(edges_on_cell(icell, 1), 0) = vertices_on_cell(icell, 2);
      vertices_on_edge(edges_on_cell(icell, 1), 1) = vertices_on_cell(icell, 1);
      vertices_on_edge(edges_on_cell(icell, 2), 0) = vertices_on_cell(icell, 3);
      vertices_on_edge(edges_on_cell(icell, 2), 1) = vertices_on_cell(icell, 2);
      ...
      edges_on_vertex(vertices_on_cell(icell, 0), 0) = edges_on_cell(icell, 0);
      edges_on_vertex(vertices_on_cell(icell, 1), 0) = edges_on_cell(icell, 0);
      edges_on_vertex(vertices_on_cell(icell, 2), 2) = edges_on_cell(icell, 1);
      edges_on_vertex(vertices_on_cell(icell, 1), 2) = edges_on_cell(icell, 1);
      edges_on_vertex(vertices_on_cell(icell, 2), 1) = edges_on_cell(icell, 2);
      edges_on_vertex(vertices_on_cell(icell, 3), 1) = edges_on_cell(icell, 2);
  });

  //parallel_for("scale_weights", 
  //    SimpleBounds<2>(nedges, 2 * maxedges), YAKL_CLASS_LAMBDA(Int iedge, Int j) {
  //    weights_on_edge(iedge, j) *= 1. / sqrt(3);
  //});
  
  parallel_for("compute_cell_arrays", 
      SimpleBounds<2>(ny, nx), YAKL_CLASS_LAMBDA(Int irow, Int icol) {
      Int icell = cellidx(icol, irow);
      ...
      for (Int j = 0; j < maxedges; ++j) {
        orient_on_cell(icell, j) = cells_on_edge(edges_on_cell(icell, j), 0) == icell ? 1 : -1;
      }
  });
  
  parallel_for("compute_edge_arrays", 
      nedges, YAKL_CLASS_LAMBDA(Int iedge) {
  ...
  parallel_for("compute_vertex_arrays", 
      nvertices, YAKL_CLASS_LAMBDA(Int ivertex) {

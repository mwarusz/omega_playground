...
void ShallowWaterModel::compute_cell_auxiliary_variables(
  ...
  OMEGA_SCOPE(dc_edge, m_mesh->m_dc_edge);
  OMEGA_SCOPE(area_cell, m_mesh->m_area_cell);
  OMEGA_SCOPE(nlayers, m_mesh->m_nlayers);
  ...
  parallel_for(
      ...
      TeamPolicy(m_mesh->m_ncells, team_size, vector_size),
      KOKKOS_LAMBDA(const TeamMember &team_member) {
        Int icell = team_member.league_rank();
        parallel_for(ThreadVectorRange(team_member, nlayers), [&](Int k) {
          Real ke = -0;
          Real div = -0;
...
void ShallowWaterModel::compute_vertex_auxiliary_variables(
  ...
  parallel_for(
      ...
      TeamPolicy(m_mesh->m_nvertices, team_size, vector_size),
      KOKKOS_LAMBDA(const TeamMember &team_member) {
        Int ivertex = team_member.league_rank();
        parallel_for(ThreadVectorRange(team_member, nlayers), [&](Int k) {
...
void ShallowWaterModel::compute_edge_auxiliary_variables(
  ...
  parallel_for(
      ...
      TeamPolicy(m_mesh->m_nedges, team_size, vector_size),
      KOKKOS_LAMBDA(const TeamMember &team_member) {
...
void ShallowWaterModel::compute_h_tendency(Real2d h_tend_cell,
  ...
  OMEGA_SCOPE(h_flux_edge, m_h_flux_edge);
  parallel_for(
      "compute_htend", TeamPolicy(m_mesh->m_ncells, team_size, vector_size),
      KOKKOS_LAMBDA(const TeamMember &team_member) {
        Int icell = team_member.league_rank();
        parallel_for(ThreadVectorRange(team_member, nlayers), [&](Int k) {
          Real accum = -0;
...
void ShallowWaterModel::compute_vn_tendency(Real2d vn_tend_edge,
  ...
  parallel_for(
      "compute_vtend", TeamPolicy(m_mesh->m_nedges, team_size, vector_size),
      KOKKOS_LAMBDA(const TeamMember &team_member) {
        Int iedge = team_member.league_rank();
        parallel_for(ThreadVectorRange(team_member, nlayers), [&](Int k) {
          Real vn_tend = -0;
          ...
          if (visc_del4 > 0) {
            Int ivertex0 = vertices_on_edge(iedge, 0);
            Int ivertex1 = vertices_on_edge(iedge, 1);
            Real visc4 = visc_del4 * mesh_scaling_del4(iedge) *
...
void ShallowWaterModel::compute_tr_tendency(Real3d tr_tend_cell,
  ...
  OMEGA_SCOPE(mesh_scaling_del4, m_mesh->m_mesh_scaling_del4);
  OMEGA_SCOPE(nlayers, m_mesh->m_nlayers);
  OMEGA_SCOPE(ncells, m_mesh->m_ncells);
  ...
  if (eddy_diff4 > 0) {
    ...
    parallel_for(
        ...
        TeamPolicy(m_mesh->m_ncells * ntracers, team_size, vector_size),
        KOKKOS_LAMBDA(const TeamMember &team_member) {
          Int l = team_member.league_rank() / ncells;
          Int icell = team_member.league_rank() % ncells;

...
void ShallowWaterModel::compute_tr_tendency(Real3d tr_tend_cell,
  ...
  if (eddy_diff4 > 0) {
    tmp_tr_del2_cell = Real3d("tmp_tr_del2_cell", ntracers, m_mesh->m_ncells,
                              m_mesh->m_nlayers);
    omega_parallel_for_outer(
        "compute_tmp_tr_del2_cell", m_mesh->m_ncells * ntracers,
        KOKKOS_LAMBDA(const TeamMember &team_member) {
  ...
  omega_parallel_for_outer(
      "compute_tr_tend", m_mesh->m_ncells * ntracers,
      KOKKOS_LAMBDA(const TeamMember &team_member) {
        Int l = team_member.league_rank() / ncells;
        Int icell = team_member.league_rank() % ncells;
